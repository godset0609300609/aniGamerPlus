"""Tests for the bt_feed_tick dramatiq actor."""
...
def test_actor_registered_on_broker() -> None:
    ...
    actor = broker.get_actor('bt_feed_tick')
    assert actor.actor_name == 'bt_feed_tick'


def test_actor_registered_with_correct_queue() -> None:
    from app.tasks.bt_feed_tick import bt_feed_tick

    assert bt_feed_tick.queue_name == 'meta'
    assert bt_feed_tick.options.get('max_retries', 0) == 0


@pytest.mark.anyio
async def test_disabled_settings_skips_service_call() -> None:
    from app.tasks.bt_feed_tick import bt_feed_tick

    coro_fn = _get_actor_coro(bt_feed_tick)
    ...
    async def fake_to_thread(fn: object, *args: object, **kwargs: object) -> None:
        raise AssertionError('run_iteration must not be called when bt_downloader is disabled')
    ...
    fake_container.bt_downloader_service.run_iteration.assert_not_called()


@pytest.mark.anyio
async def test_enabled_settings_calls_run_iteration_via_to_thread() -> None:
    from app.tasks.bt_feed_tick import bt_feed_tick
    ...
    assert calls == [fake_container.bt_downloader_service.run_iteration]
