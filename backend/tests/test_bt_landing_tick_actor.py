"""Tests for the bt_landing_tick dramatiq actor."""
...
def test_actor_registered_on_broker() -> None:
    ...
    actor = broker.get_actor('bt_landing_tick')
    assert actor.actor_name == 'bt_landing_tick'


def test_actor_registered_with_correct_queue() -> None:
    from app.tasks.bt_landing_tick import bt_landing_tick

    assert bt_landing_tick.queue_name == 'downloads'
    assert bt_landing_tick.options.get('max_retries', 0) == 0
...
async def test_disabled_settings_skips_worker_call() -> None:
    ...
    coro_fn = _get_actor_coro(bt_landing_tick)
    ...
    fake_container.bt_landing_worker.run_iteration.assert_not_called()
...
async def test_enabled_settings_calls_run_iteration_via_to_thread() -> None:
    ...
    assert calls == [fake_container.bt_landing_worker.run_iteration]
